...
from timm.models.layers import PatchEmbed, Mlp, DropPath, trunc_normal_, lecun_normal_
from timm.models.pretrained import generate_default_cfgs
from timm.models.registry import register_model

from prompt import EPrompt
from attention import PreT_Attention
# from utils_adam import CosineLinear
...
class Block(nn.Module):
    ...
    def __init__(
            ...
            drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, attn_layer=Attention, use_lae=False):
        super().__init__()
...
def dataset_idx2text(dataset='Split-CIFAR100'):
    if dataset=='Split-CIFAR100':
        import json
        with open('continual_datasets/dataset_infos.json') as f:
            classes = json.load(f)['cifar100']['features']['fine_label']['names']
        idx2text = {i:classes[i] for i in range(len(classes))}
        return idx2text
            

def text_preprocess(target, dataset):
    """
    @params target: torch tensor (int): (bs,)
    return text_target: list (bs,)
    """
    idx2text = dataset_idx2text(dataset=dataset)
    text_target = ['a photo of '+idx2text[trg_idx.item()] for trg_idx in target]
    return text_target
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            class_token=True, no_embed_class=False, 
            pre_norm=False,
            fc_norm=None, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.,
            ...
            eval_prototype_clf=False, original_model=False, args=None,
            ):
        """
        Args:
        ...
        self.g_prompt_layer_idx = g_prompt_layer_idx # 0 1
        ...
        self.use_prefix_tune_for_g_prompt = use_prefix_tune_for_g_prompt # set to true
        
        self.use_e_prompt = use_e_prompt
        self.e_prompt_layer_idx = e_prompt_layer_idx # 2 3 4
        num_e_prompt = len(self.e_prompt_layer_idx) if self.e_prompt_layer_idx is not None else 0
        self.use_prefix_tune_for_e_prompt = use_prefix_tune_for_e_prompt # set to true
        ...
        if use_g_prompt and g_prompt_length is not None and len(g_prompt_layer_idx) != 0: # set to true
            if not use_prefix_tune_for_g_prompt: # set to false
                ...
                if same_key_value: # set to false
                    ...
                    # (g_prompt_applied_layers, key&values, g_prompt_nums, ...)
                    if prompt_init == 'zero':
                        self.g_prompt = nn.Parameter(torch.zeros(g_prompt_shape))
                    elif prompt_init == 'uniform':
                        self.g_prompt = nn.Parameter(torch.randn(g_prompt_shape))
        ...
        if use_e_prompt and e_prompt_layer_idx is not None: # set to true
            self.e_prompt = EPrompt(length=prompt_length, embed_dim=embed_dim, embedding_key=embedding_key, prompt_init=prompt_init,
                    ...
                    num_heads=num_heads, same_key_value=same_key_value, coda_prompt=args.coda_prompt, 
                    num_coda_prompt=args.num_coda_prompt, feat_prompt=args.feat_prompt, 
                    num_feat_prompt=args.ptm_num_feat_prompt if original_model else args.num_feat_prompt,
                    softmax_feat=args.softmax_prompt, original_model=original_model, args=args)
            print('e-prompt is init')
        
        if not (use_g_prompt or use_e_prompt): # set to false
            attn_layer = Attention
        elif not (use_prefix_tune_for_g_prompt or use_prefix_tune_for_e_prompt): # set to false
            # Prompt tunning
            attn_layer = Attention
        else: # set to true
            # Prefix tunning
        ...
        if self.prompt_pool:
            if not self.use_prefix_tune_for_g_prompt: # set to true
                self.total_prompt_len += g_prompt_length * len(self.g_prompt_layer_idx) # 5 * 2
            if not self.use_prefix_tune_for_e_prompt: # set to true
                self.total_prompt_len += prompt_length * top_k * len(self.e_prompt_layer_idx) # 5*1*3
                # prompt_length for E-prompt
        ### prompt settings #################################################################################################
        #####################################################################################################################

        #####################################################################################################################
        ### clip settings #################################################################################################
        self.clip_emb = args.clip_emb
        if self.clip_emb:
            self.clip_proj = nn.Linear(512,embed_dim)
            self.clip_emb_norm = norm_layer(embed_dim//num_heads)
            self.num_heads = num_heads
        ### clip settings #################################################################################################
        #####################################################################################################################
        ...
        self.sep_specialization = args.sep_specialization
        self.eval_known_prompt = args.eval_known_prompt
        ...
        self.blocks = nn.Sequential(*[
            block_fn(
                ...
                attn_layer=attn_layer, use_lae=False)
        ...
        self.task_agnoistic_head = False
        self.clip_text_head = False
        self.eval_prototype_clf = False
        
        if args.task_agnostic_head:
            self.task_agnoistic_head = True
            # self.head = nn.Linear(self.embed_dim, 1) # Logit
            self.mlp = Mlp(in_features=self.embed_dim,  hidden_features=self.embed_dim, 
                            act_layer=nn.GELU, drop=0. ) # Logit
            self.head = nn.Linear(self.embed_dim*2, 1)
            self.sigmoid = nn.Sigmoid()
        #####################################################################################################################
        ### clip text encoder head ##########################################################################################
        elif args.clip_text_head:
            # import clip
            self.clip_text_head = True
            self.logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / 0.07))
            self.head = nn.Linear(self.embed_dim, num_classes) if num_classes > 0 else nn.Identity()
            self.dataset_nb_class = args.nb_classes
            self.trg_embs = self.load_trg_embs(args.dataset)
            
            # self.head = clip.load("ViT-B/16",)[0] # 0 index is clip model
        ### clip text encoder head ##########################################################################################
        #####################################################################################################################
        ...
        # print('self.fc_norm', self.fc_norm)

    def load_trg_embs(self,dataset):
        with torch.no_grad():
            clip_model, _ = clip.load("ViT-B/16", device=self.device)
            text_clip_emb = text_preprocess(torch.arange(self.dataset_nb_class), dataset) # (Cls,): strings
            text_clip_emb = clip.tokenize(text_clip_emb).to(self.device) # (Cls, 77)
            text_clip_emb = clip_model.encode_text(text_clip_emb).detach().cpu() # (Cls, D)
        return text_clip_emb
    ...
    def forward_features(self, x, task_id=-1, target=None, cls_features=None, aug_cls_features=None,
                         clip_emb=None,train=False, ):
        """
        clip_emb: (bs,D)
        """

        x = self.patch_embed(x) #(bs,ps,D)
        x = self._pos_embed(x)
        x = self.norm_pre(x)
        # if self.cls_token is not None:
        #     x = torch.cat((self.cls_token.expand(x.shape[0], -1, -1), x), dim=1) #(bs,1+ps,D)
        
        # x = self.pos_drop(x + self.pos_embed)

        if self.grad_checkpointing and not torch.jit.is_scripting(): # set to false
            x = checkpoint_seq(self.blocks, x)
        else:
            if self.use_g_prompt or self.use_e_prompt: # set to true
                if self.use_e_prompt: #self.use_prompt_mask and train: # set to true
                    if train or self.eval_known_prompt:
                        if self.sep_specialization:
                            prompt_mask = task_id.to(x.device) # (bs,1)
                        else:
                            start = task_id * self.e_prompt.top_k
                            end = (task_id + 1) * self.e_prompt.top_k
                            
                            single_prompt_mask = torch.arange(start, end).to(x.device)
                            prompt_mask = single_prompt_mask.unsqueeze(0).expand(x.shape[0], -1) # (bs, #top_k=1)
                            if end > self.e_prompt.pool_size: # pool_size <- 10 for e-prompt
                                prompt_mask = None
                    else:
                        prompt_mask = None
                    
                    
                    res = self.e_prompt(x, target=target, prompt_mask=prompt_mask, cls_features=cls_features, aug_cls_features=aug_cls_features,
                            task_id=task_id,)
                    e_prompt = res['batched_prompt'] # (bs,top_k,D) # D and C are exchangable
                else:
                    prompt_mask = None
                    g_prompt_counter = -1
                    res = dict()
                ...
                if self.clip_emb:
                    ...
                    clip_bs, clip_dim = clip_emb.size()
                    # num_heads = g_prompt.size(-2)
                    clip_emb = clip_emb.view(clip_bs, self.num_heads,
                                 clip_dim//self.num_heads).unsqueeze(1).unsqueeze(1)
                    # print(clip_emb.size())
                    
                for i, block in enumerate(self.blocks):
                    if i in self.g_prompt_layer_idx: # [0 1]
                        if self.use_prefix_tune_for_g_prompt: # set to true
                            g_prompt_counter += 1
                            # Prefix tunning, [B, 2, g_prompt_length, num_heads, embed_dim // num_heads]
                            idx = torch.tensor([g_prompt_counter] * x.shape[0]).to(x.device) # (bs,)
                            g_prompt = self.g_prompt[idx]
                            if self.clip_emb:
                                # clip_emb: (bs, 1,1,H,D/H)
                                g_prompt = g_prompt + clip_emb
                                g_prompt = self.clip_emb_norm(g_prompt)
                            
                        else:
                            g_prompt=None
                        x = block(x, prompt=g_prompt)
                    
                    elif i in self.e_prompt_layer_idx: # [2 3 4]
                        e_prompt_counter += 1
                        ...
                        x = block(x)
            else: # no prompt or prefix tuning
                x = self.blocks(x)
    ...
    def forward_head(self, res, cls_features=None, pre_logits: bool = False):
        x = res['x']
        if self.class_token and self.head_type == 'token': # set to true
            if self.task_agnoistic_head:
                prompt_tok = x[:, 0] 
            if self.prompt_pool: # set to true
                x = x[:, self.total_prompt_len] # CLS token: (bs, 1, D)
            else:
                x = x[:, 0]
        ...
        x = self.fc_norm(x) # identity now

        if self.task_agnoistic_head:
            x = self.mlp(x)
            prompt_tok = self.mlp(prompt_tok)
            logits = (x * prompt_tok).sum(-1,).unsqueeze(-1)
        
        elif self.eval_prototype_clf:
            if self.training :
                logits = self.head(x) # normalize -> matmul
            else: # EVAL
                # print("EVAL-ing")
                if self.merge_pt:
                    logits = self.proto_head(torch.cat([x, cls_features], dim=-1))
                else:
                    logits = self.proto_head(x)
        
        else:
            logits = self.head(x) # if clip text encoder, the head is frozen.
        res['logits'] = logits # fc
        if self.task_agnoistic_head:
            res['logits'] = self.sigmoid(logits) # fc
        
        return res

    def forward(self, x, task_id=-1, cls_features=None, aug_cls_features=None, clip_emb=None, 
                    train=False, target=None,):
        res = self.forward_features(x, task_id=task_id, target=target, cls_features=cls_features, aug_cls_features=aug_cls_features,
            clip_emb=clip_emb, train=train,)
        res = self.forward_head(res, cls_features=cls_features)
        return res
